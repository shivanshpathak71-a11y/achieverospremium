...
export function CinematicPlayer({ lecture, onEnded }: {
  lecture: Lecture;
  onEnded?: () => void;
}) {
  ...
  return (
    <div ref={containerRef} className="relative w-full aspect-video bg-black rounded-2xl overflow-hidden group select-none"
      onMouseMove={showControlsTemp} onMouseLeave={() => { if (!menuView && playing) setShowControls(false); }}
      ...
      <video ref={videoRef} src={lecture.video_url || undefined} poster={lecture.thumbnail_url || undefined}
        ...
        onLoadedMetadata={() => { const v = videoRef.current; if (v) { setDuration(v.duration); v.playbackRate = speed; } }}
        onTimeUpdate={() => { const v = videoRef.current; if (!v || seeking) return; setPosition(v.currentTime); const bf = v.buffered; if (bf.length > 0) setBuffered(bf.end(bf.length - 1)); }}
        ...
        {showControls && (
          <motion.div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-4 pt-12 pb-3"
            ...
            <div className="relative h-1.5 rounded-full bg-white/20 cursor-pointer mb-3 group/seek" onClick={handleSeekClick}>
              ...
              <div className="absolute inset-y-0 left-0 rounded-full bg-pink-500" style={{ width: `${duration > 0 ? (position / duration) * 100 : 0}%` }} />
              <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-pink-500 shadow-md opacity-0 group-hover/seek:opacity-100 transition-opacity"
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button onClick={() => setMenuView(menuView ? null : 'main')} className={`h-8 px-2.5 rounded-lg flex items-center justify-center gap-1.5 transition-colors text-xs font-semibold ${menuView ? 'text-pink-400 bg-white/15' : 'text-white hover:bg-white/15'}`} aria-label="Settings">
                  <Settings className={`w-4 h-4 transition-transform duration-200 ${menuView ? 'rotate-45' : ''}`} />
                </button>
                <AnimatePresence>
                  {menuView && (
                    <motion.div className="absolute bottom-10 right-0 w-44 bg-black/90 backdrop-blur-xl rounded-xl border border-white/10 p-1.5 shadow-2xl"
                      ...
                      {menuView === 'main' && (
                        <>
                          <button onClick={() => setMenuView('speed')} className="w-full px-3 py-2 rounded-xl text-xs text-white hover:bg-white/10 flex items-center justify-between transition-colors">
                            <span className="flex items-center gap-2"><Settings className="w-3.5 h-3.5" /> Speed</span>
                          ...
                          <button onClick={() => setMenuView('quality')} className="w-full px-3 py-2 rounded-xl text-xs text-white hover:bg-white/10 flex items-center justify-between transition-colors">
                            ...
                            <span className="text-pink-400">{quality}</span>
                          </button>
                        </>
                      )}
                      {menuView === 'speed' && (
                        <>
                          ...
                          {SPEEDS.map((s) => (
                            <button key={s} onClick={() => { setSpeed(s); const v = videoRef.current; if (v) v.playbackRate = s; setMenuView(null); }}
                              className={`w-full px-3 py-2 rounded-xl text-xs text-left flex items-center justify-between transition-colors ${speed === s ? 'text-pink-400 bg-white/10' : 'text-white hover:bg-white/10'}`}>
                        ...
                        <>
                          ...
                          {QUALITIES.map((q) => (
                            <button key={q} onClick={() => { setQuality(q); setMenuView(null); }}
                              className={`w-full px-3 py-2 rounded-xl text-xs text-left flex items-center justify-between transition-colors ${quality === q ? 'text-pink-400 bg-white/10' : 'text-white hover:bg-white/10'}`}>
